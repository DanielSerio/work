...
import { CategoriesTable, db, eq, inArray, NOW } from "astro:db";
...
export const PATCH: APIRoute<any, { id: string; }> = async ({ request, params }) => {
  try {
    ...
    const results = await db.update(CategoriesTable).set(parsed).where(eq(CategoriesTable.id, entityID)).returning();
...
export const DELETE: APIRoute<any, { id: string; }> = async ({ params }) => {
  try {
    ...
    const results = await db.update(CategoriesTable).set({
      ...
      eq(CategoriesTable.id, entityID)
    ).returning();
